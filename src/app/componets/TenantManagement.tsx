...
export default function TenantManagement() {
  ...
  useEffect(() => {
    ...
    if (!container) return;
    const inner = container.querySelector(".tenant-image-inner") as HTMLElement;
    if (!inner) return;
  ...
  return (
    <section 
      ref={sectionRef}
      className="container lg:max-w-6xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-0 py-10 mt-12 md:mt-20 overflow-hidden"
    >
      ...
      >
        ...
        <div className="lg:w-1/2 w-full space-y-6">
          <div>
            <div className="h-20 w-20 text-2xl flex items-center justify-center mb-2 rounded-full bg-[#F8EBEB]">
              <FontAwesomeIcon icon={faCog} className="text-[#A00500] text-3xl" />
            </div>
          </div>

          <h1 className="text-3xl md:text-4xl font-semibold">Tenant Management Made Easy</h1>

          <ul className="text-[#7A7A7A] text-lg space-y-3">
            {[
              "Paperless tenant onboarding in minutes",
              "Manage all tenant details, notices & documents",
              "Monitor dues, rent status and receipts",
              "Stay notified of pending/expiring documents"
            ].map((item, index) => (
        ...
        <div className="lg:w-1/2 w-full flex justify-center">
          <div ref={imageRef} className="perspective-[1000px] w-full max-w-[600px]">
            <div className="tenant-image-inner transition-transform duration-300 ease-out will-change-transform">
              <Image
                src="/4/3.png"
                alt="Tenant Management Illustration"
                width={600}
                height={400}
